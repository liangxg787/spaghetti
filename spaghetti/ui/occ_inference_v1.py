...
import spaghetti.utils.rotation_utils
...
class Inference:
    ...
    def disentanglement_plot(self, item_a: int, item_b: int, z_in_a, z_in_b, a_inclusive: bool = True,
        ...
        with torch.no_grad():
            ...
            if bottom:
                ...
                z_in = [self.model.sdformer.forward_split(self.model.sdformer.forward_upper(z_))[0][0] for z_
                        in z_in]
            else:
                z_in = merge_z(zh)
    ...
    def load_file(self, info_path, disclude: Optional[List[int]] = None):
        ...
        for i, key in enumerate(keys):
            ...
            split.append(
                len(split) + torch.ones(len(info['ids'][key]), dtype=torch.int64, device=self.device))
    ...
    def get_intersection_z(z_a: T, z_b: T) -> T:
        ...
        if diff_a.shape[0] != diff_b.shape[0]:
            padding = torch.zeros(abs(diff_a.shape[0] - diff_b.shape[0]), device=z_a.device,
                                  dtype=torch.bool)
            if diff_a.shape[0] > diff_b.shape[0]:
    ...
    @models_utils.torch_no_grad
    def get_attention_faces(self, mesh: T_Mesh, zh: T, mask: TN = None, fixed_z: TN = None,
                            alpha: TN = None):
    ...
    @models_utils.torch_no_grad
    def interpolate_from_files(self, item_a: Union[str, int], item_b: Union[str, int], num_mid: int,
                               res: int,
        ...
        for i, alpha_ in enumerate(torch.linspace(0, 1, num_mid)):
            ...
            if not files_utils.is_file(out_path):
                ...
                files_utils.export_list(colors.tolist(),
                                        f"{self.opt.cp_folder}/{folder}/ff_{counter + i:03d}_faces")
            logger.reset_iter()
    ...
    @models_utils.torch_no_grad
    def plot(self, prefix: Union[str, int], verbose=False, interpolate: bool = False, res: int = 200,
             size: int = -1,
    ...
    def mix_file(self, path: str, z_idx, replace_inds, num_samples: int, res=220):
        ...
        for i in range(num_samples):
            ...
            gmms_ = [torch.cat((item, item_new[i].unsqueeze(0)), dim=2) for item, item_new in
                     zip(gmms, gmms_new)]
    ...
    def plot_occ_one(self, i, gmms=[], z=[], z_base=[], res=200, fixed_items=[], folder_name='',
                     use_item_id=True, verbose=True):
        gmm_ = [gmms[j][i].unsqueeze(0) for j in range(len(gmms))]
        # mesh = self.get_mesh_(z[i], res)
        mesh = self.get_mesh(z[i], res, [gmm_])
        if use_item_id and fixed_items is not None:
            name = f'{fixed_items[i]}'
        elif len(z) == 1:
            name = ''
        else:
            name = f'{i:02d}'
        ...
        if mesh is not None:
            ...
            files_utils.save_pickle(z_base[i].detach().cpu(),
    ...
    def plot_occ_(self, z: Union[T, TS], z_base, gmms: Optional[List[TS]],
                  fixed_items: Optional[Union[T, List[str]]],
                  folder_name: str, res=200, verbose=False, use_item_id: bool = False):
        gmms = gmms[-1]

        i_s = [i for i in range(len(z))]
        plot_occ_one_ = partial(self.plot_occ_one, gmms=gmms, z=z, z_base=z_base, res=res,
                                fixed_items=fixed_items,
                                folder_name=folder_name, use_item_id=use_item_id, verbose=verbose)

        cpu_num = 2
        Parallel(n_jobs=cpu_num, backend='loky', verbose=10)(
            delayed(plot_occ_one_)(batch) for batch in tqdm(i_s))

    @models_utils.torch_no_grad
    def plot_(self, folder_name: str, verbose=False, res: int = 200, names: Optional[List[str]] = None):
        if names is not None:
            # fixed_items = self.get_samples_by_names(names)
            fixed_items = torch.tensor([i for i in range(self.opt.dataset_size)])
        else:
            fixed_items = torch.randint(low=0, high=self.opt.dataset_size, size=(10,))
        zh_base, _, gmms, _ = self.model.get_embeddings(fixed_items.to(self.device))
        zh, attn_b = self.model.merge_zh(zh_base, gmms)
        self.plot_occ_(zh, zh_base, gmms, names, folder_name, verbose=verbose, res=res,
                       use_item_id=True)
